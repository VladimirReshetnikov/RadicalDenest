...
\input{tables/fixed_A}
\input{tables/fixed_BC}
\input{tables/fixed_DE}
\input{tables/fixed_JK}
\input{tables/fixed_LQ}

\subsection{Probing the corrected version for misses}
\label{sec:probes}
After the battery above, four further scripts
(\code{harness/probe\_gaps\_1..4.wl}, transcripts in \code{logs/}) ran 136
denestable or control inputs drawn from the literature guide and from
constructed identities through \code{Strad} with a 400\,s wall limit. No
wrong value was returned. Of the 111 denestable inputs not already evaluated
by the kernel itself, 96 were denested to the expected depth: multiquadratic
sums with up to four independent square roots, depth-3 inputs, the Ramanujan
and Honsbeek identities, mixed-index sums such as
$\sqrt{2+2\sqrt2\sqrt[3]3+\sqrt[3]9}=\sqrt2+\sqrt[3]3$, complex radicands,
and cube, fifth and seventh roots of positive elements of quadratic fields.
The 15 misses fall into five families (Table~\ref{tab:probes}); the file
\code{src/corrected/KNOWN\_GAPS.md} gives every input, the verbose diagnosis
and a proposed fix for each.

\begin{table}[h]\centering\small
\begin{tabularx}{\textwidth}{@{}p{0.29\textwidth} X p{0.3\textwidth}@{}}
\toprule Family (inputs) & Example & Cause\\\midrule
A. Principal root of a negative radicand, index $\ge5$ (8) &
$(41-29\sqrt2)^{1/5}=(-1)^{1/5}(\sqrt2-1)$ &
the core solves only the factor of $x^r-\rho$ shared with the principal
root's minimal polynomial; for $r\ge5$ \code{Solve} returns \code{Root}
objects or nested surds, certified but not cheaper. Index 3 and 4 work.\\[3pt]
B. Composite index, answer of larger degree than the radicand (4) &
$\sqrt[6]{7\sqrt[3]{20}-19}=\sqrt[3]{5/3}-\sqrt[3]{2/3}$ &
the certified candidate is $\gamma^{1/3}$ with $\gamma=\beta^3\in\Q(\rho)$,
same depth as the input, hence discarded; no index reduction, no recursion
on rejected candidates.\\[3pt]
C. Factorizer extracts a factor with irrational root (1) &
$\sqrt[6]{1296+880\sqrt2+720\sqrt3+528\sqrt6}$ &
\code{Factorc} pulls out $16$, leaving $2^{2/3}(\cdots)^{1/6}$ of degree 12;
with \code{"Factor"->False} the input is denested in 0.1\,s.\\[3pt]
D. Cancellation across summands (1) &
$\sqrt{1+\sqrt3}+\sqrt{3+3\sqrt3}-\sqrt{10+6\sqrt3}=0$ &
summands are independent problems and are individually non-denestable
(Section~\ref{sec:future}, item 3).\\[3pt]
E. Time budget not enforced inside a trial (1) &
$\sqrt{(\sqrt2+\sqrt3+\sqrt5+\sqrt7+\sqrt{11})^2}$ expanded &
one \code{PolynomialGCD}/\code{Solve} over eleven surds exceeds 150\,s;
the budget is checked only between trials, so guarantee (iv) of
Section~\ref{sec:guarantees} fails here. Four primes take 2.9\,s.\\
\bottomrule
\end{tabularx}
\caption{Denestable inputs that the corrected version leaves unchanged
(5 September 2026).}
\label{tab:probes}
\end{table}
