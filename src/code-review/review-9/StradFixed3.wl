...
(* StradFixed3.wl -- proposed research revision, September 2026.
...
resolveOptions[head_Symbol, raw_List] := Module[{rules, names, unknown, cfg, bad},
   ...
   bad = Join[
     ...
     Select[{"MaxTrials", "MultiplierCap", "MaxTraceEntries", "MaxRecursion", "Patience",
       "MaxCharacterRank", "MaxCharacterPatterns", "DiscriminantBatchCap"}, ! nonnegativeIntegerQ[cfg[#]] &],
...
standalone[body_, failure_] := If[TrueQ[$active], body,
   Block[{$active = True, $cfg = Association[Options[Strad]], $deadline = AbsoluteTime[] + 20,
     $stats = newStats[], $limits = <||>, $trace = {}, $records = {}, $memo = <||>, $recursion = 0, $inProgress = <||>, $lastCertificateMethod = "None", $Assumptions = True},
...
accept[candidate_, target_, incumbent_, method_String] := Module[{},
   ...
   If[certify[candidate, target] === "Equal",
    ...
    If[Length[$records] < $cfg["MaxTraceEntries"],
     AppendTo[$records, <|"Before" -> target, "After" -> candidate, "Method" -> method, "EqualityMethod" -> $lastCertificateMethod|>]];
...
Strad[e_, all : (True | False), opts___] := invoke[e, Strad, {"AllLevels" -> all, opts}, False, False];
Strad[e_, opts___] := invoke[e, Strad, {opts}, False, False];
DenestRadicals[e_, all : (True | False), opts___] := invoke[e, DenestRadicals, {"AllLevels" -> all, opts}, False, False];
DenestRadicals[e_, opts___] := invoke[e, DenestRadicals, {opts}, False, False];
DenestCore[e_, opts___] := invoke[e, DenestCore, {opts}, False, True];
DenestReport[e_, all : (True | False), opts___] := invoke[e, DenestReport, {"AllLevels" -> all, opts}, True, False];
DenestReport[e_, opts___] := invoke[e, DenestReport, {opts}, True, False];
